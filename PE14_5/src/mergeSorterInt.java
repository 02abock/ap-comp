public class mergeSorterInt{
	public mergeSorterInt(phoneNumber[] anArray)
	{
		a=anArray;
	}
	public void sort() {
		 ...
		 mergeSorterInt firstSorter=new mergeSorterInt(first);
		 mergeSorterInt secondSorter=new mergeSorterInt(second);
		 firstSorter.sort();
		 secondSorter.sort();
		 merge(first,second);
	}
	private void merge(phoneNumber[] first, phoneNumber[] second)
	{
		...
		while (iFirst< first.length&& iSecond< second.length) {
			if (first[iFirst].getNum()>(second[iSecond].getNum())) {
				a[j]=first[iFirst];
				iFirst++;
			}
			else {
